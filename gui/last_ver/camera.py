        ...
        try:
            while self.is_running:
                ...
                for result in results:
                    if result.masks is not None:
                        ...
                        for i, mask in enumerate(resized_masks):
                            ...
                            # obj_id가 None이거나 이미 current_frame_objects에 존재하는 경우 무시
                            if obj_id is None or obj_id in current_frame_objects:
                                print("메롱")
                                continue
                ...
                for obj_id, obj_data in current_frame_objects.items():
                    ...
                    if self.candidate_objects[obj_id] == 2:
                    ...
                    if self.candidate_objects[obj_id] >= 15 and obj_id not in self.confirmed_objects:
                        ...
                        if obj_id == None or self.last_confirmed_obj_id == obj_id:
                            print("약오르징")
                            continue
                        else:
                    ...
                    if gw.first_object_weight is not None:
                        ...
                        if abs(weight_difference) >= 10:
                            # last_confirmed_obj_id가 None이거나 이미 current_frame_objects에 존재하는 경우 무시
                            if self.last_confirmed_obj_id is None:
                                print(f"요거닝~~~~~~~~~~~~~~~~ㅋㅋㅋㅋ{self.last_confirmed_obj_id}!!!!!!!!!!!!!!!!!!!!{weight_difference}")
                                continue
                            else :
                            # if self.last_confirmed_obj_id == None
                                print(f"이거닝~~~~~~~~~~~~~~~~{self.last_confirmed_obj_id}!!!!!!!!!!!!!!!!!!!!{weight_difference}")
                                print(f"[INFO] 무게 변화 감지: {self.confirmed_objects[self.last_confirmed_obj_id]['obj_name']}, 무게 증가량: {weight_difference:.2f} g")

                                # print(f"정체가 뭐야 {last_key}")


                                # if last_key == self.last_confirmed_obj_id:
                                #     print("나는 같아아!!!!!!!!!!!!!")
                                # else:
                                if self.last_confirmed_obj_id in self.confirmed_objects:
                                    print("나는 다르지 않아!!!!!!")
                                    print(self.last_confirmed_obj_id)
                                    print("볼륨업뎃하러옴옴")
                                    print(type(self.last_confirmed_obj_id))

                                    # last_key = next(reversed(self.confirmed_objects.keys()))
                                    # print(f"너는 누구니{current_frame_objects.keys()}")
                                    print(f'나 지금 : {next(reversed(self.confirmed_objects.keys()))}')
                                    print(f'필요한건 이거라구 : {self.last_confirmed_obj_id}')
                                    if next(reversed(self.confirmed_objects.keys())) != self.last_confirmed_obj_id:
                                        print(f'current 리스트 현재상황 : {current_frame_objects}')
                                        print(f'틀려서 들어왔는데 나 커런트트 : {next(reversed(self.confirmed_objects.keys()))}')
                                        print(f'틀려서 들어왔는데 나 라스트트 : {self.last_confirmed_obj_id}')
                                        print("화면이랑 나랑 달라 나는 다른애야!!!! ")
                                        break
                                    else:
                                        print("오 같네?")
                                    ...
                                    if self.last_confirmed_obj_id not in list(current_frame_objects.keys()):
                                        ...
                                        if current_volume is None:
                                            print(f"Tlqkfzz{current_volume}")
                                        else:
                                            print(f"부피모야?'{current_volume}")
                                            ...
                                            print(f"업데이트된 객체: id: {self.last_confirmed_obj_id}, name: {self.confirmed_objects[self.last_confirmed_obj_id]['obj_name']}, Volume: {current_volume:.1f} cm³, Updated Weight: {update_weight:.2f} g")
                                            ...
                                            if food_info:
                                                ...
                                                print(f"[INFO] {obj_name} 칼로리 추가됨: {new_calories:.2f} kcal")
                                                print(f"현재까지 총 칼로리: {self.total_calories:.2f} kcal")
