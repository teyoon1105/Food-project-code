...
class DepthVolumeCalculator:
    ...
    def visualize_results(self, cropped_image, mask, object_name, volume, conf, mask_indices):
        ...
        for text, (text_width, text_height) in zip(texts, text_sizes):
            ...
            cv2.putText(mask, text, (x,y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color=(0,0,0), thickness = 2)
            y += spacing  # 줄 간격 추가


        
        return cv2.addWeighted(cropped_image, 0.7, mask, 0.3, 0) # 이미지 합성
