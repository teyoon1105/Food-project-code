...
video_name = 'test_' + model_name + '.avi'
...
def main():
    ...
    try:
        while True:
            ...
            sleep_time = max(0, abs(frame_interval - elapsed_time))
            print(f"Frame {frame_count}, Elapsed: {elapsed_time:.3f}s, Sleep: {sleep_time:.3f}s")
        ...
        print(f"Total Frames: {frame_count}, Total Time: {total_elapsed_time:.2f}s, Actual FPS: {actual_fps:.2f}")
        
        # 저장된 비디오를 처리 속도에 맞게 FPS 조정
        if actual_fps > 0:  # 실제 FPS가 0보다 클 때만 조정
            updated_video_name = 'updated_' + video_name
            updated_out = cv2.VideoWriter(updated_video_name, fourcc, actual_fps, (width, height))
            
            # 기존 비디오 내용을 새 비디오에 복사
            cap = cv2.VideoCapture(video_name)
            while cap.isOpened():
                ret, frame = cap.read()
                if not ret:
                    break
                updated_out.write(frame)
            cap.release()
            updated_out.release()

            print(f"Updated video saved at: {updated_video_name}")
