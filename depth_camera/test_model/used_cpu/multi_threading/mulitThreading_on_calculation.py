...
class DepthVolumeCalculator:
    ...
    def process_object(self, result, idx, cropped_depth, depth_intrin, save_depth, roi_color):
        """
        ...
        try:
            ...
            for y, x in zip(y_indices, x_indices):
                z_cm = cropped_depth[y, x] / 10.0
                base_z_cm = save_depth[y, x] / 10.0
                
                if z_cm > 25 and base_z_cm > 25: # 최소값 필터링
                    height_cm = max(0, base_z_cm - z_cm)
                    pixel_area_cm2 = (z_cm ** 2) / (depth_intrin.fx * depth_intrin.fy)
                    volume += (pixel_area_cm2 * height_cm)
