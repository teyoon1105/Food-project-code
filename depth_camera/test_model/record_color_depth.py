...
output_video_path = 'test_' + model_name + '.avi' # 비디오 제목
...
def preprocess_depth_for_visualization(depth_image, roi_points):
    """깊이 데이터를 시각화를 위해 컬러맵으로 변환"""
    cropped_depth = crop_roi(depth_image, roi_points)
    depth_colormap = cv2.applyColorMap(
        cv2.convertScaleAbs(cropped_depth, alpha=0.5), 
        cv2.COLORMAP_JET
    )
    return cropped_depth, depth_colormap

# ---- 메인 처리 루프 ----
def main():
    global save_depth, output_video_path
    align = initialize_camera()
    processed_frames = []  # 처리된 프레임 저장 리스트
    depth_frames = []  # 깊이 스트림 처리된 프레임
    frame_timestamps = []  # 프레임 타임스탬프 저장
    start_time = time.time()

    try:
        while True:
            frame_start_time = time.time()  # 현재 프레임 시작 시간
            ...
            cropped_depth, depth_colormap = preprocess_depth_for_visualization(depth_image, ROI_POINTS)
            ...
            # 결과 프레임 저장
            processed_frames.append(blended_image)
            depth_frames.append(depth_colormap)
            frame_timestamps.append(frame_start_time - start_time)           
        ...
        save_video_with_timestamps(depth_frames, frame_timestamps, "depth_" + output_video_path)
        print(len(processed_frames), len(depth_frames))
