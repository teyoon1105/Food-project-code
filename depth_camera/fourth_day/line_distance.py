...
try:
    while True:
        ...
        # cv2.line(img, (525,430), (525,430), (255,0,0), 5)
        cv2.line(img, (520,340), (747,340), (255,0,0), 2)
        cv2.line(img, (632,182), (632, 500), (0,255,255), 3)
        ...
        for result in results:
            if result.masks is not None:  # 탐지된 객체에 마스크가 있는 경우
                ...
                for i, mask in enumerate(masks):
                    ...
                    line_x2, line_y2 = 747 - x1, 340 - y1

                    # 직선의 길이 세로
                    # line_x1, line_y1 = 632 - x1, 182 - y1
                    # line_x2, line_y2 = 632 - x1, 500 - y1

                    length_width = math.sqrt((line_x1-line_x2)**2 + (line_y1-line_y2)**2)

                    if cropped_depth_image[line_x1, line_y1] > 0 and cropped_depth_image[line_x2, line_y2] > 0:
                        z_cm_dot1 = cropped_depth_image[line_x1, line_y1] / 10
                        z_cm_dot2 = cropped_depth_image[line_x2, line_y2] / 10

                        depth_intrin = aligned_depth_frame.profile.as_video_stream_profile().intrinsics

                        pixel_size_dot1 = z_cm_dot1 / depth_intrin.fx  # cm/pixel
                        pixel_size_dot2 = z_cm_dot2 / depth_intrin.fx # cm/pixel

                        real_length_width1 = length_width * pixel_size_dot1 # cm
                        real_length_width2 = length_width * pixel_size_dot2 # cm
                        
                        # 두 점의 평균값 사용
                        real_length_width = (real_length_width1 + real_length_width2) / 2

                        print(real_length_width1, real_length_width2)
                        
                        print(f"실제 가로 길이: {real_length_width:.2f} cm")
                        # print(f"실제 세로 길이: {real_length_width:.2f} cm")
                    ...
                    min_depth_mm = 10  # 최소 깊이 기준 설정
                    for i in range(len(x_indices)):
                        # 크롭된 이미지 좌표 기준으로 깊이 값 계산
                        pixel_y = y_indices[i]
                        pixel_x = x_indices[i]
                        
                        # 각 픽셀까지 거리를 z_cm으로 저장(mm단위로)
                        z_cm = cropped_depth_image[pixel_y, pixel_x] / 10
                        # 각 픽셀까지 거리를(ROI) z_cm로 저장(mm단위로)
                        base_depth_cm = save_depth[pixel_y, pixel_x] / 10

                        # 높이값 예외처리(0이 아닌 값)
                        if z_cm > min_depth_mm and base_depth_cm > 40 and np.isfinite((z_cm * z_cm)/(depth_intrin.fx * depth_intrin.fy)):
                            height_cm = base_depth_cm - z_cm  # 높이 계산 (cm)

                            # 픽셀 면적 계산 (mm²)
                            pixel_area_cm2 = (z_cm * z_cm)/(depth_intrin.fx * depth_intrin.fy)
                            total_area_cm2 += pixel_area_cm2
                    ...
                    cv2.putText(all_colored_mask, f"{object_name}:V:{total_volume_cm3:.2f}cm_3,A:{total_area_cm2:.2f}cm_2",
