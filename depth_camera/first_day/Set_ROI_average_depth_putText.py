...
try:
    ...
    while True:
        ...
        if len(roi_pts) == 2:  # 두 점이 선택되었으면 ROI 계산 및 표시
            ...
            # text 넣어주기
            cv2.putText(img, "{} cm".format(int(average_distance_cm)) ,(x1, y1), 0, 1, (0,0,0), 2)
                

            cropped_image = img[y1:y2, x1:x2]
            resize_crob_img = cv2.resize(cropped_image, (640,480))
            cv2.imshow('Cropped Image', resize_crob_img) #새로운 창 생성
