                ...
                for i, mask in enumerate(masks):
                    ...
                    for i in range(len(x_indices)):
                        ...
                        if z_cm > min_depth_cm and base_depth_cm > 0 and np.isfinite((depth_intrin.fx * depth_intrin.fy) / (z_cm * z_cm)):
                            ...
                            # 픽셀 면적 계산 (cm^2)
                            pixel_area_cm2 = (depth_intrin.fx * depth_intrin.fy) / (z_cm * z_cm) * (0.01 * 0.01)  # 픽셀 단위에서 cm 단위로 변환

                            # voxel 부피 계산 (cm^3)
                            voxel_volume_cm3 = pixel_area_cm2 * height_cm

                            total_volume_cm3 += voxel_volume_cm3


                    min_x, max_x = np.min(x_indices), np.max(x_indices)
                    min_y, max_y = np.min(y_indices), np.max(y_indices)
                    label_x, label_y = min_x, min_y - 10 
                    cv2.putText(all_colored_mask, f"{object_name}:V:{total_volume_cm3:.2f}cm_3,H{height_cm:.2f}cm",
